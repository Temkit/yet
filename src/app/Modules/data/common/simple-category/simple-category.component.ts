...
export class SimpleCategoryComponent implements OnInit {
  ...
  @Input() set config(val) {
    if (val) {
      ...
      this.render = this.S3Service.getSpec(
        ...
        map(data => {
          if (data) {
            this.oldTree = JSON.parse(JSON.stringify(data.tree));
            this.tree = data.tree;
  ...
  deleteItem(node) {
    ...
    this.database.data.filter(function(el) {
      return el != null;
    });
  ...
  saveTree() {
    if (this.isNewTree) {
      const item = JSON.parse(JSON.stringify(this.objectConfig.Key));
      item["active"] = -1;
      item["tree"] = this.database.data;

      this.__c_
        .create$(this.objectConfig.TableName, item, this.objectConfig.Region)
        .subscribe(data => {});
    } else {
      this.__u_
        .updateConstructor$(
